...
Write-Host "Step 2: Checking admin-service database (localhost:5432/clan_platform)..." -ForegroundColor Yellow
Write-Host "Container: admin-service-postgres" -ForegroundColor Gray
Write-Host ""
docker exec admin-service-postgres psql -U postgres -d clan_platform -c "SELECT COUNT(*) as user_count FROM usersetup_basic;"
Write-Host ""
docker exec admin-service-postgres psql -U postgres -d clan_platform -c "SELECT id, username, email, employee_id, created_at FROM usersetup_basic ORDER BY created_at DESC LIMIT 5;"
Write-Host ""
...
$latestUser = docker exec admin-service-postgres psql -U postgres -d clan_platform -t -c "SELECT username FROM usersetup_basic ORDER BY created_at DESC LIMIT 1;" | ForEach-Object { $_.Trim() }
if ($latestUser) {
...
Write-Host "  Database: clan_platform" -ForegroundColor Gray
Write-Host "  Container: admin-service-postgres" -ForegroundColor Gray
Write-Host ""
